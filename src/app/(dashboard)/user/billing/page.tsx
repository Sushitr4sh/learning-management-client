...
const UserBillingPage = () => {
  ...
  // Rename isLoading so we don't get confused with user isLoaded
  const { data: transactions, isLoading: isLoadingTransactions } =
    useGetTransactionsQuery(user?.id || "", {
      // Make sure this query doesn't get called until user id exist
      skip: !isLoaded || !user,
    });

  // This doesn't really do anything because we only have one payment type(stripe), just in case if we want to expand this application
  const filteredData =
