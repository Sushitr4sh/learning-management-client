...
interface InitialStateTypes {
  courseEditor: {
    ...
    selectedSectionIndex: number | null; // Identify which section we're modifying.
...
export const globalSlice = createSlice({
  ...
  initialState,
  reducers: {
    // Basically a function that affects our global state.
    setSections: (state, action: PayloadAction<Section[]>) => {
      state.courseEditor.sections = action.payload; // payload is the new information that is going to replace what was previosly there.
    },
    openChapterModal: (
      state,
